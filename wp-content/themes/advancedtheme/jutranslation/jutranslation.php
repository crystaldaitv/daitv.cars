...
namespace Joomunited\ADVANCEDTHEME\Jutranslation;
...
class Jutranslation
{
    ...
    /**
     * The main extension php file
     *
     * @var string $main_extension_file
     */
    public static $main_extension_file;

    /**
     * Type of extension plugin or them
     *
     * @var string $type
     */
    public static $type;
     ...
     *
     * @param string $main_extension_file Main extension file
     ...
     * @return void
     */
    public static function init($main_extension_file, $extension_slug, $extension_name, $text_domain, $language_file, $type = 'plugin')
    {
        self::$main_extension_file = $main_extension_file;
        self::$extension_slug      = $extension_slug;
        self::$extension_name      = $extension_name;
        self::$text_domain         = $text_domain;
        self::$language_file       = $type === 'plugin' ? plugin_dir_path(self::$main_extension_file) . $language_file : get_template_directory() . DIRECTORY_SEPARATOR . $language_file;
        self::$type                = $type;
        ...
        add_filter(self::$extension_slug . '_get_addons', function ($addons) {
            $addon                          = new \stdClass();
            $addon->main_extension_file     = Jutranslation::$main_extension_file;
    ...
    public static function getInput()
    {
        ...
        wp_enqueue_script(
            'jutranslation',
            (self::$type === 'plugin' ? plugin_dir_url(self::$main_extension_file) : get_template_directory_uri() . '/') . 'jutranslation/assets/js/jutranslation.js',
            array(),
            ADVANCED_THEME_VERSION
        );
        wp_enqueue_style(
            'jutranslation',
            (self::$type === 'plugin' ? plugin_dir_url(self::$main_extension_file) : get_template_directory_uri() . '/') . 'jutranslation/assets/css/jutranslation.css'
        );
        ...
        foreach ($languages as &$language) {
            if (str_replace('-', '_', $language->languageCode) === 'en_US') {
                ...
                $file = WP_CONTENT_DIR . DIRECTORY_SEPARATOR . 'languages' . DIRECTORY_SEPARATOR . (self::$type === 'plugin' ? 'plugins' : 'themes');
        ...
        foreach ($addons as $addon) {
            ...
            if (isset($addon->type) && $addon->type === 'theme') {
                $extension_data = wp_get_theme();
            } else {
                $extension_data = get_plugin_data($addon->main_extension_file);
            }
        ...
        echo '<table id="jutranslations-languages" class="table table-striped" >
                    <thead >
                        <tr>
                            <th ' . (count($addons) > 1 ? '' : 'style="display:none;"') . '>Extension</th>
                            <th>Language</th>
        ...
        foreach ($languages as $language) {
            ...
            echo '<span class="latest_version"><img src="' .
                 esc_attr((self::$type === 'plugin' ? plugin_dir_url(self::$main_extension_file) : get_template_directory_uri() . '/') . 'jutranslation/assets/images/radio.svg') .
                 '" alt="loading"/></span><br/>';
            ...
            if (!isset($julanguages[$language->extension->extension_slug])) {
                $extension_data = self::$type === 'plugin' ? get_plugin_data($language->extension->main_extension_file) : wp_get_theme($language->extension->main_extension_file);
                ...
                $julanguages[$language->extension->extension_slug]['extension_version'] = $extension_data['Version'];
    ...
    protected static function saveStrings()
    {
        ...
        $extension = $_POST['slug'];
        if (!$extension) {
            echo json_encode(array('status' => 'error', 'message' => 'extension empty'));
            die();
        }
        if (!preg_match('/^[a-zA-Z-_]+$/', $extension)) {
            echo json_encode(array('status' => 'error', 'message' => 'invalid extension slug'));
            die();
        }

        $addons = apply_filters(self::$extension_slug . '_get_addons', array());
        if (!isset($addons[$extension])) {
            echo json_encode(array('status' => 'error', 'message' => 'extension not found'));
            die();
        }

        //Get the file to write to
        $destination       = $_POST['destination'];
        $file              = WP_CONTENT_DIR . DIRECTORY_SEPARATOR . 'languages' . DIRECTORY_SEPARATOR . ($addons[$extension]->type === 'theme' ? 'themes' : 'plugins');
        $file              .= DIRECTORY_SEPARATOR . $addons[$extension]->text_domain . '-' . str_replace('-', '_', $language);
        $extension_version = '';
        $revision          = '';
        switch ($destination) {
            case 'override':
            ...
            case 'edition':
                ...
                if (isset($mo->headers['Version']) && $mo->headers['Version']) {
                    ...
                    $extension_data    = get_plugin_data($addons[$extension]->main_extension_file);
                    $extension_version = $extension_data['Version'];
                }
    ...
    protected static function getTranslation()
    {
        ...
        $file = $addons[$extension]->language_file;
        $mo   = new \MO();
        $mo->import_from_file($file);
        ...
        $file = WP_CONTENT_DIR . DIRECTORY_SEPARATOR . 'languages' . DIRECTORY_SEPARATOR . ($addons[$extension]->type === 'theme' ? 'themes' : 'plugins');
        $file .= DIRECTORY_SEPARATOR . $addons[$extension]->text_domain . '-';
    ...
    protected static function showViewForm()
    {
        ...
        $file = WP_CONTENT_DIR . DIRECTORY_SEPARATOR . 'languages' . DIRECTORY_SEPARATOR . ($addons[$extension]->type === 'theme' ? 'themes' : 'plugins');
        $file .= DIRECTORY_SEPARATOR . $addons[$extension]->text_domain . '-' . str_replace('-', '_', $language) . '.mo';
        if (!file_exists($file)) {
        ...
        if (isset($addons[$extension]->type) && $addons[$extension]->type === 'theme') {
            $extension_data = wp_get_theme($addons[$extension]->main_extension_file);
        } else {
            $extension_data = get_plugin_data($addons[$extension]->main_extension_file);
        }
        ...
        echo '<input type="hidden" name="extension" value="' . esc_attr($addons[$extension]->extension_slug) . '" />';
        ...
        echo '<div style="text-align:center"><img src="' .
             esc_attr((self::$type === 'plugin' ? plugin_dir_url(self::$main_extension_file) : get_template_directory_uri() . '/')) . 'jutranslation/assets/images/preview_loader.gif"></div>';
    ...
    {
        //Only for our extension and addons
        $addons = apply_filters(self::$extension_slug . '_get_addons', array());
